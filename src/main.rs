...
fn main() {
    ...
    let cmd = match matches.subcommand() {
        Some(sub) => (cli::SubCommand::from_str(sub.0).unwrap(), sub.1),
        None => {
...
fn handle_cmd((cmd, args): (cli::SubCommand, &ArgMatches), data: &mut rmbrs::Remembers) -> bool {
    match (cmd, args.subcommand()) {
        ...
        (cli::SubCommand::Links, sub_cmd) => match sub_cmd {
            Some(more_sub_cmd) => match (more_sub_cmd.0, more_sub_cmd.1) {
                ("add", aargs) => {
        ...
        (cli::SubCommand::Todos, sub_cmd) => match sub_cmd {
            Some(more_sub_cmd) => match (more_sub_cmd.0, more_sub_cmd.1) {
                ("add", aargs) => {
        ...
        (cli::SubCommand::Timers, sub_cmd) => match sub_cmd {
            Some(more_sub_cmd) => match (more_sub_cmd.0, more_sub_cmd.1) {
                ("add", aargs) => {
